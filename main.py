...
# ==========================================
# 2. الدوال المساعدة ونظام الترقيم
# ==========================================
...
def build_text(original_text, source_id, is_wholesale=False):
    ...
    # القناة P - استخراج السعر بذكاء من كلمة price
    if prefix == "P":
        ...
        if p_match:
            ...
            # تنظيف النص من الكود القديم وسعر المصدر
    ...
    # منطق استخراج السعر للقناة P وللقنوات الأخرى
    found_price_val = None
    if prefix == "P":
        p_price_match = re.search(r'price\s*(\d+)', normalize_numbers(original_text), re.IGNORECASE)
    ...
    if not found_price_val:
        ...
        if online_match: found_price_val = int(online_match.group(1))
            ...
            nums = [int(n) for n in re.findall(r'(\d+)', original_text) if 10 <= int(n) <= 2000]
            if nums: found_price_val = min(nums)
        else:
            any_p = re.search(r'(\d+)\s*(?:جنيه|ج)', processed_text)
            if any_p: found_price_val = int(any_p.group(1))
            else:
                nums = [int(n) for n in re.findall(r'(\d+)', original_text) if 10 <= int(n) <= 2000]
                if nums: found_price_val = nums[0]

    new_price_val = mapping.get(found_price_val, "")
    new_price_str = f"{new_price_val} ج" if new_price_val else ""
    
    # تنظيف السطور التي تحتوي على أسعار قديمة
    patterns = [r'.*(?:اونلاين|online).*', r'.*(?:سعر القطعه|القطعه بـ|price|بسعر|جمله|جملة).*', r'.*(?:بدل|عرض خاص|عرض).*', r'^\d+\s*(?:ج|جنيه)?\s*$']
...
# ==========================================
# 3. نظام النشر (بدون تجميع) والسحب التاريخي
# ==========================================
async def send_to_targets(client, messages, source_id):
    ...
    try:
        # إرسال كل الصور/الفيديوهات بشكل منفصل (بدون تجميع)
        for m in messages:
            if m.photo:
        ...
        # إرسال النص في النهاية ليفصل بين الموديلات
...
async def fetch_history(client):
    print("🔎 جاري سحب الشغل القديم من 10-04-2026...")
    ...
    print("✅ تم الانتهاء من سحب القديم.")

# ==========================================
# 4. معالجة الرسائل الجديدة والتعديلات
# ==========================================
media_groups = {}
...
async def main_handler(client, message):
    if message.media_group_id:
        if message.media_group_id not in media_groups:
            ...
            await asyncio.sleep(3) # انتظار تجميع الألبوم
...
async def edit_handler(client, message):
    ...
    res = cursor.fetchone()
    if res:
        await client.edit_message_text(WHOLESALE_CHANNEL, res[0], message.text)
...
@web_app.route('/')
def home(): return "Bot Organizing & Fixes Active!"
