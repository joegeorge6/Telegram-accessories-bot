...
# ==========================================
# 2. الدوال المساعدة وفلتر الإعلانات
# ==========================================
...
# فلتر الكلمات المحظورة (إعلانات الشركة)
...
def build_text(original_text, source_id, msg_date):
    ...
    patterns = [r'.*(?:اونلاين|online).*', r'.*(?:سعر القطعه|الالقطعه بـ|price|بسعر|جمله|جملة).*', r'.*(?:بدل|بكام|عرض خاص|عرض|بس).*', r'.*(?:الكود|السعر|بسعر).*[:：].*', r'^[A-Z]+\d+.*', r'^[\W\s]*\d+[\W\s]*$']
...
async def send_to_targets(client, messages, source_id):
    ...
    # إذا كان المنشور إعلاناً (retail_text == None) توقف فوراً ولا ترسل الميديا
    ...
    try:
        ...
        if retail_text: await safe_send(client.send_message, RETAIL_CHANNEL, retail_text)
...
# 4. تشغيل البوت
# ==========================================
app = Client("retail_v17", api_id=API_ID, api_hash=API_HASH, session_string=SESSION_STRING, in_memory=True)
...
@web_app.route('/')
def home(): return "Retail Pro Bot v17.0 Active!"
