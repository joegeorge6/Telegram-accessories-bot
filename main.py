...
START_DATE = parse_date(os.environ.get("START_DATE", ""), datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0))
...
# ==========================================
# 2. الدوال المساعدة وفلاتر التنظيف
# ==========================================
...
def extract_real_price(text):
    ...
    # تجاهل الأرقام المرتبطة بالقياسات
...
def build_text(original_text, source_id, msg_date):
    ...
    # القاعدة الجديدة: لو المنشور بدون نص نهائياً -> ارجع نص فارغ (لا ترسل كود ولا سعر)
    ...
    # تنظيف النص الأصلي من كود وسعر المصدر
    ...
    if prefix == "P":
        # لو فيه وصف (نص): ارسل النص كما هو + كودك وسعرك (بدون ترجمة الكود الأصلي)
        if description:
            return f"{description}\n\nالكود : 🔖 {my_code}\nبسعر : 💰 {price_str_ar} ج 🔥"
        # لو كود وسعر فقط: ارسل القالب التسويقي
...
async def send_to_targets(client, messages, source_id):
    ...
    if retail_text is None: return # تجاهل الإعلانات تماماً
    ...
    try:
        ...
        # إرسال النص فقط إذا كان غير فارغ
        if retail_text != "":
...
# 4. تشغيل البوت
# ==========================================
app = Client("retail_v19", api_id=API_ID, api_hash=API_HASH, session_string=SESSION_STRING, in_memory=True)
...
async def main_handler(client, message):
    ...
    else: await send_to_targets(client, [message], message.chat.id)

web_app = Flask(__name__)
@web_app.route('/')
def home(): return "Retail Pro Bot v19.0 Active!"
