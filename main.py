...
async def save_counter(key, value):
    await asyncio.to_thread(_sync_save_counter, key, value)
...
def _sync_is_msg_processed(msg_id, source_id):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def _sync_mark_msg_as_processed(msg_id, source_id):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    try:
...
async def is_msg_processed(msg_id, source_id):
    return await asyncio.to_thread(_sync_is_msg_processed, msg_id, source_id)

async def mark_msg_as_processed(msg_id, source_id):
    await asyncio.to_thread(_sync_mark_msg_as_processed, msg_id, source_id)
...
def build_text_original(original_text, source_id, msg_date, current_num):
    ...
    if re.search(r'HEMA\s*STORE', original_text, re.IGNORECASE): return ""
    if re.search(r'https?://', original_text, re.IGNORECASE): return None
    
    norm_text = normalize_numbers(original_text)
    if any(word in norm_text for word in BLOCK_KEYWORDS): return None
    if is_emoji_only(norm_text): return ""
    ...
    for line in lines:
        ...
        if re.match(r'^كود\s+\d+', line, re.IGNORECASE): continue
        ...
        if not has_special_offer:
            ...
            if match:
                ...
                if re.search(r'(?:جملة|جمله|اونلاين|online)', line, re.IGNORECASE): continue
    ...
    for name, prices in product_prices.items():
        final_price = None
        if "online" in prices: final_price = prices["online"]
        elif "direct" in prices: final_price = prices["direct"]
        elif "jomla" in prices: final_price = prices["jomla"]
        if final_price:
    ...
    for line in norm_text.split('\n'):
        ...
        if re.match(r'^[A-Z]+-\d+$', line): continue
        ...
        if re.match(r'^[A-Z]+\s*\d+', line, re.IGNORECASE):
            ...
            if found_price_val is None: found_price_val = p
            continue
        ...
        if re.search(r'\bL\s*\.?\s*E\b', line, re.IGNORECASE) or re.search(r'\bLe\b', line): continue

        if re.search(r'عرض', line, re.IGNORECASE) and not re.search(r'سعر', line, re.IGNORECASE):
            if re.search(r'عرض\s+\d+\s*(?:mm|cm|مم|سم|متر|ملي|inch|بوصة)', line, re.IGNORECASE): pass
            else:
                line = re.sub(r'^.*?عرض\s*\S*\s*', '', line, flags=re.IGNORECASE).strip()
                if not line: continue

        if re.search(r'(?:للحجز|طلب الاوردر|للطلب)', line, re.IGNORECASE): continue
        if re.search(r'\b01\d{9}\b', line): continue

        if not found_price_val:
            fallback_match = re.search(r'\bب\s*(\d+)\s*(?:ج|جنيه)', line)
            if fallback_match: fallback_prices.append(int(fallback_match.group(1)))
        ...
        if is_number_emoji_line(line): continue

        if line and len(line.split()) == 1:
            if re.match(r'^[\u0600-\u06FF]+$', line): pass
            elif not any(c.isascii() and c.isalpha() for c in line): continue

        if len(line) <= 3 and not any(c.isascii() and c.isalpha() for c in line): continue
...
# ============================================================
# معالج قناة ayselstore55 – الإصدار 3.9.30 (17 معالجاً مستقلاً)
# ============================================================
def aysel_processor(text, msg_date, current_num, source_id):
    ...
    if any('بروشات البحر' in line for line in lines) and any(re.search(r'ب\s*\d+\s*جنيه', line) for line in lines):
        ...
        description_lines = []
        for line in lines:
            match = re.search(r'ب\s*(\d+)\s*جنيه', line, re.IGNORECASE)
            if match:
                price = int(match.group(1))
                clean_line = re.sub(r'\s*ب\s*\d+\s*جنيه.*$', '', line, flags=re.IGNORECASE).strip()
                if clean_line:
    ...
    # =============================================
    # معالج 2: سلسله شيك (سعر القطعه فقط)
    # =============================================
    if any('سلسله' in line for line in lines) and any('سعر القطعه' in line for line in lines) and not any('عرض خاص' in line for line in lines):
        price = None
        description_lines = []
        for line in lines:
            match = re.search(r'سعر\s*القطعه\s*[:：]?\s*(\d+)', line, re.IGNORECASE)
            if match:
                price = int(match.group(1))
                continue
            description_lines.append(line)
    ...
    if any('برسنج' in line for line in lines) and any('سعر الفرده' in line for line in lines):
        ...
        price_label = None
        for line in lines:
            match = re.search(r'(سعر\s*الفرده\s*الواحده)\s*ب\s*(\d+)\s*جنيه', line, re.IGNORECASE)
            if match:
                price_label = match.group(1).strip()
                price = int(match.group(2))
                continue
            description_lines.append(line)
        if price is not None and price_label is not None:
            description = "\n".join(description_lines).strip()
            retail_price = RETAIL_MAPPING.get(price, price)
            price_ar = convert_to_arabic_numbers(retail_price)
    ...
    if any('غوايش' in line for line in lines) and any('الطقم' in line for line in lines):
        ...
        description_lines = []
        for line in lines:
            match = re.search(r'سعر\s*الطقم\s*(\d+)', line, re.IGNORECASE)
            if match:
                price = int(match.group(1))
                clean_line = re.sub(r'\s*سعر\s*الطقم\s*\d+', '', line, flags=re.IGNORECASE).strip()
                if clean_line:
                    description_lines.append(clean_line)
                continue
            description_lines.append(line)
    ...
    if any('كولكشن خواتم' in line for line in lines) and any('عرض' in line for line in lines):
        ...
        for line in lines:
            ...
            if match:
                price = int(match.group(1))
                continue
            if re.search(r'ختم\s*AS', line, re.IGNORECASE):
                line = re.sub(r'\s*ختم\s*AS', '', line, flags=re.IGNORECASE).strip()
            if line:
                description_lines.append(line)
        if price is not None:
            description = "\n".join(description_lines).strip()
    ...
    if any('توينز' in line for line in lines) and any('سعر التوينز' in line for line in lines):
        ...
        description_lines = []
        for line in lines:
            match = re.search(r'سعر\s*التوينز\s*(\d+)', line, re.IGNORECASE)
            if match:
                price = int(match.group(1))
                continue
            description_lines.append(line)
    ...
    if any('ايطاليان برسلت' in line for line in lines):
        ...
        description_lines = []
        for line in lines:
            if 'ايطاليان برسلت' in line:
                description_lines.append(line)
                continue
            match1 = re.search(r'(الاسوره\s*السلفر\s*بالاتشارمز)\s*ب\s*(\d+)', line, re.IGNORECASE)
            match2 = re.search(r'(الاسوره\s*الجولد\s*بالاتشارمز)\s*(\d+)', line, re.IGNORECASE)
            if match1:
                labels.append(match1.group(1).strip())
                prices.append(int(match1.group(2)))
            elif match2:
                labels.append(match2.group(1).strip())
                prices.append(int(match2.group(2)))
            else:
                description_lines.append(line)
        if prices and labels and len(prices) == 2:
            description = "\n".join(description_lines).strip()
            result_lines = [description, f"الكود : 🔖 {my_code}"]
    ...
    # (يستخدم العرض الخاص، يحذف السعرين)
    # =============================================
    if any('سلسله حروف' in line for line in lines) and any('سعر القطعه' in line for line in lines) and any('عرض خاص' in line for line in lines):
        ...
        description_lines = []
        for line in lines:
            match = re.search(r'عرض\s*خاص\s*(\d+)', line, re.IGNORECASE)
            if match:
    ...
    # (يستخدم العرض الخاص، يحذف السعرين)
    # =============================================
    if any('سلسله تيفانى' in line for line in lines) and any('سعر القطعه' in line for line in lines) and any('عرض خاص' in line for line in lines):
        price_offer = None
        description_lines = []
        for line in lines:
            match = re.search(r'عرض\s*خاص\s*(\d+)', line, re.IGNORECASE)
    ...
    # =============================================
    # معالج 10: سلسله حمصات (سعر القطعه 100 + عرض خاص 85)
    # (يستخدم العرض الخاص، يحذف السعرين، يحذف N-)
    # =============================================
    if any('سلسله حمصات' in line for line in lines) and any('سعر القطعه' in line for line in lines) and any('عرض خاص' in line for line in lines):
        price_offer = None
        description_lines = []
        for line in lines:
            match = re.search(r'عرض\s*خاص\s*(\d+)', line, re.IGNORECASE)
    ...
    # (يستخدم العرض الخاص، يحذف السعرين)
    # =============================================
    if any('خاتم "شداد"' in line for line in lines) and any('سعر القطعة' in line for line in lines) and any('عرض خاص' in line for line in lines):
        price_offer = None
        description_lines = []
        for line in lines:
            match = re.search(r'عرض\s*خاص\s*جدًا\s*[:：]?\s*(\d+)', line, re.IGNORECASE)
            if match:
                price_offer = int(match.group(1))
                continue
    ...
    # (يستخدم العرض الخاص، يحذف السعرين)
    # =============================================
    if any('السلسله التريندى' in line for line in lines) and any('سعر القطعه' in line for line in lines) and any('عرض خاص' in line for line in lines):
        price_offer = None
        description_lines = []
        for line in lines:
            match = re.search(r'عرض\s*خاص\s*(\d+)', line, re.IGNORECASE)
    ...
    if total_match:
        ...
        for line in lines:
            ...
            if match:
                ...
                price = int(match.group(2))
                if product_name and 15 <= price <= 2000:
    ...
    # =============================================
    # معالج 14: سلسله قلب ستالس (سعر القطعه 75 + عرض خاص 50)
    # (يستخدم العرض الخاص، يحذف السعرين)
    # =============================================
    if any('سلسله قلب' in line for line in lines) and any('سعر القطعه' in line for line in lines) and any('عرض خاص' in line for line in lines):
        price_offer = None
        description_lines = []
        for line in lines:
            match = re.search(r'عرض\s*خاص\s*(\d+)', line, re.IGNORECASE)
    ...
    # =============================================
    # معالج 15: سلسله فراشه (سعر القطعه 50 فقط)
    # =============================================
    ...
    # =============================================
    # معالج 16: أنسيال دورين + رمز B-003
    # =============================================
    if any('أنسيال دورين' in line for line in lines) and any('سعر القطعة' in line for line in lines):
        price = None
        description_lines = []
        for line in lines:
            if re.search(r'الرمز\s*[:：]', line, re.IGNORECASE):
                continue
            match = re.search(r'سعر\s*القطعة\s*[:：]?\s*(\d+)', line, re.IGNORECASE)
            if match:
                price = int(match.group(1))
                continue
            description_lines.append(line)
    ...
    if any('طقم' in line for line in lines) and any('سلسله' in line for line in lines) and any('انسيال' in line for line in lines):
        ...
        for line in lines:
            # حذف سعر الإجمالي
            if re.search(r'سعر\s*القطعتين\s*مع\s*بعض', line, re.IGNORECASE):
                continue
            # استخراج منتجين
            match_s = re.search(r'(سلسله\s*ستالس)\s*سعر\s*القطعه\s*(\d+)', line, re.IGNORECASE)
            match_a = re.search(r'(انسيال\s*ستالس)\s*سعر\s*القطعه\s*(\d+)', line, re.IGNORECASE)
            if match_s:
                name = match_s.group(1).strip()
                price = int(match_s.group(2))
                if 15 <= price <= 2000:
                    products.append((name, price))
                continue
            elif match_a:
                name = match_a.group(1).strip()
                price = int(match_a.group(2))
                if 15 <= price <= 2000:
                    products.append((name, price))
                continue
            description_lines.append(line)
        if products and len(products) == 2:
            ...
            for name, price in products:
                retail = RETAIL_MAPPING.get(price, price)
                price_ar = convert_to_arabic_numbers(retail)
                # تصحيح "ستالس" إلى "استانلس" في الاسم
                name_fixed = name.replace('ستالس', 'استانلس')
                result_lines.append(f"{name_fixed} سعر القطعه  : 💰 {price_ar} ج 🔥")
            return "\n".join(result_lines)

    # =============================================
    # معالج خاص: النصوص التي تحتوي على سعر القطعه + عرض خاص (قاعدة عامة)
    # (يستخدم العرض الخاص دائماً، يحذف السعرين)
    # =============================================
    # نبحث عن وجود سعر القطعه وعرض خاص في أي نص
    has_individual_price = any(re.search(r'سعر\s*القطعه|سعر\s*القطعة', line, re.IGNORECASE) for line in lines)
    has_special_offer = any(re.search(r'عرض\s*خاص', line, re.IGNORECASE) for line in lines)
    if has_individual_price and has_special_offer:
        price_offer = None
        description_lines = []
        for line in lines:
...
# ============================================================
# معالج قناة hebaNor (شنط) – الإصدار 3.9.30
# ============================================================
...
# ============================================================
# معالج قناة irona44 (شنط) – الإصدار 3.9.30
# ============================================================
...
# ============================================================
# معالج قناة ronybags (شنط) – الإصدار 3.9.30
# ============================================================
...
# ============================================================
# معالج قناة I (إكسسوارات) – الإصدار 3.9.30
# ============================================================
...
def organizer_processor(text, msg_date, current_num, source_id):
    ...
    for line in desc_text.split('\n'):
        ...
        if not line_stripped:
            continue
        if re.search(rf'\b{price}\b', line_stripped):
            cleaned = re.sub(r'[\U0001F300-\U0001F5FF\U0001F600-\U0001F64F\U0001F680-\U0001F6FF\U0001F1E0-\U0001F1FF\U00002702-\U000027B0\U000024C2-\U0001F251\U0001F900-\U0001F9FF\U0001FA00-\U0001FA6F\U0001FA70-\U0001FAFF\U00002600-\U000026FF\U0000FE00-\U0000FE0F\U0000200D\U00002B50\U00002764\U0001F004\U0001F0CF]+', '', line_stripped)
            if cleaned.strip().isdigit():
                price_lines_to_remove.append(line_stripped)
                continue
        ...
        if re.search(r'ب\s*\d+', line_stripped, re.IGNORECASE):
            price_lines_to_remove.append(line_stripped)
            continue
        if re.search(r'سعر\s*\d+', line_stripped, re.IGNORECASE):
            price_lines_to_remove.append(line_stripped)
            continue
    ...
    for line in desc_text.split('\n'):
        ...
        if not line_stripped:
            continue
        if line_stripped in price_lines_to_remove:
            continue
...
def get_processor(source_id):
    if isinstance(source_id, str) and source_id in PROCESSOR_MAP: return PROCESSOR_MAP[source_id]
    if isinstance(source_id, int) and source_id in PROCESSOR_MAP: return PROCESSOR_MAP[source_id]
    if isinstance(source_id, int) and str(source_id) in PROCESSOR_MAP: return PROCESSOR_MAP[str(source_id)]
    if source_id in PROCESSOR_MAP: return PROCESSOR_MAP[source_id]
...
async def safe_send(client, messages, source_id):
    ...
    if retail_text is None:
        await mark_msg_as_processed(messages[0].id, source_id)
        return
    ...
    try:
        ...
        if retail_text != "":
            ...
            if raw_caption:
                ...
                await save_counter(counter_key, current_num)
...
async def fetch_history(client):
    ...
    for channel in SOURCE_CHANNELS:
        ...
        async for msg in client.get_chat_history(channel, limit=6000):
            ...
            if (END_DATE_LIMIT and m_date > END_DATE_LIMIT) or await is_msg_processed(msg.id, channel):
                continue
...
async def main_handler(client, message):
    if message.photo and is_screenshot(message.photo): return
    if message.poll or await is_msg_processed(message.id, message.chat.id): return
...
@web_app.route('/')
def home():
    return "Retail Pro Bot v3.9.30 (Aysel: 17 handlers, fixed special offer priority) Ready!"

async def start_bot():
    global channel_counters
    await init_db()
    channel_counters = load_counters()
    print("🚀 Retail Pro Bot v3.9.30 يبدأ... (إصلاح أولوية العرض الخاص في Aysel)")
    await app.start()
