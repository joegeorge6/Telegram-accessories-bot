...
# التعديل: لو التاريخ فارغ، ابدأ من بداية اليوم الحالي (الساعة 00:00)
...
# 4. تشغيل البوت
# ==========================================
app = Client("retail_v16", api_id=API_ID, api_hash=API_HASH, session_string=SESSION_STRING, in_memory=True)
...
async def main_handler(client, message):
    if message.media_group_id:
        ...
        if gid not in media_groups:
            media_groups[gid] = []
            media_groups[gid].append(message) # إضافة أول رسالة فوراً
            await asyncio.sleep(10) # انتظار تجميع الباقي
...
@web_app.route('/')
def home(): return "Retail Pro Bot v16.0 Active!"
