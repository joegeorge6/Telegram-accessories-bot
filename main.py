...
def build_text(original_text, source_id, my_code):
...
# ==========================================
# 3. نظام النشر والسحب التاريخي (قطاعي فقط)
# ==========================================
async def send_to_targets(client, messages, source_id):
    ...
    retail_text = build_text(main_msg.caption or main_msg.text, source_id, my_code)
...
# 4. تشغيل البوت
# ==========================================
app = Client("retail_v2", api_id=API_ID, api_hash=API_HASH, session_string=SESSION_STRING, in_memory=True)

@app.on_message(filters.chat(SOURCE_CHANNELS) & ~filters.forwarded)
async def main_handler(client, message):
    # للرسائل الجديدة: انتظار تجميع الألبوم أو إرسال منفرد
    if not message.media_group_id:
        await send_to_targets(client, [message], message.chat.id)

web_app = Flask(__name__)
@web_app.route('/')
def home(): return "Retail Professional Bot is Running!"
